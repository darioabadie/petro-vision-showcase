{{ config(materialized='table', order_by='operator_slug') }}

WITH raw_ops AS (
    SELECT DISTINCT operator_raw
    FROM {{ ref('stg_energy__well_production') }}
...
FROM raw_ops o
LEFT JOIN {{ ref('seed_operator_aliases') }} a
    ON lower(trim(a.operator_raw)) = lower(trim(o.operator_raw))
    AND a.review_status = 'approved'
)
SELECT
    {{ slugify('operator_raw') }}                                       AS operator_slug,
