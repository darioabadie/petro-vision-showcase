{{ config(
    ...
    order_by='(well_id, month_date)',
    partition_by='toYear(month_date)',
) }}
...
FROM {{ ref('stg_energy__well_production') }} s
JOIN {{ ref('dim_operator') }} o ON s.operator_raw = o.operator_raw