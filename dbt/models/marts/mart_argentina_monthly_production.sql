...
max_month AS (
    SELECT max(month_date) AS m FROM {{ ref('fact_well_monthly_production') }}
)
SELECT
    ...
    if(
        a.month_date <= {{ pvm_cutoff_date() }}
        AND (a.month_date < mm.m OR (a.month_date = mm.m AND mm.m <= {{ pvm_cutoff_date() }})),
        1, 0
    )                                                     AS is_complete
FROM agg a
